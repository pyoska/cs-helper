...
export async function generateMetadata({ params }) {
  ...
  const canonicalUrl = `https://cshelper.kr/${getSlug(companyName)}`;
  const title = `${cleanName} 전화번호 및 연결 팁 - CS 고객센터 도우미`;
  const description = `${cleanName} 고객센터 대표번호(${company?.phone || ""}) 연결 후, ARS 안내 멘트를 끝까지 들을 필요 없이 즉시 상담원과 통화할 수 있는 단축번호 치트키를 지금 확인해 보세요.`;
      ...
      <main className="flex-grow container mx-auto px-4 py-8 max-w-4xl">
        ...
        <section className="bg-white rounded-3xl shadow-xl overflow-hidden border border-white">
          <div className="p-8 sm:p-12">
            ...
            <div className="mb-3 p-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 shadow-2xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-800">공식 발표 자료 기준 정리 가이드</span>
              </div>
